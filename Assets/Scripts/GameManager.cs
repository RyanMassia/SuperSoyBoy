...
public class GameManager : MonoBehaviour
{
   ...
   public void SaveTime(decimal time)
    ...
    {
        var existingLevelRoot = GameObject.Find("Level"); // runs the level, if a level gameobjecty already exists delete and make a new one  
        Destroy(existingLevelRoot);
        var levelRoot = new GameObject("Level");

        var levelFileJsonContent = File.ReadAllText(selectedLevel); // reeads the json file data 
        ...
        // 1 finds the Camera lerp script
        var camSettings = FindObjectOfType<CameraLerpToTransform>();
        // 2 if its finds it populates setting for the speed bounds and tracking 
        if (camSettings != null)
